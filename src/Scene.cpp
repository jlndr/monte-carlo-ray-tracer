	...
	for(Triangle LT : l.getLightTriangles()) {
		...
		for(int i = 0; i < 3; ++i){
			...
			if(materialType == OREN_NAYAR) {
				double sigma = INT_MAX; //roughness factor
...
void Scene::drawRoom() {
	...
	Material GrayOren{ColorDbl{1.0f, 1.0f, 1.0f}, OREN_NAYAR};
	//ROOF
	//((-3, 0, 5))
	//(0, 6 , 5)
	...
	//bigboi
	// vec3 tBotFront{6, 1, -2};
	// vec3 tBotRight{8, -3, -2};
	// vec3 tBotLeft{8 , 5, -2};
	// vec3 tTop{8, 1, 2.5};
	...
	// spheres.push_back(Sphere{vec3{9.0f, 0.0f, -1.0f}, 2.5f, GrayOren});
	// spheres.push_back(Sphere{vec3{9.0f, 3.0f, 3.0f}, 1.5f, YellowPerf});
